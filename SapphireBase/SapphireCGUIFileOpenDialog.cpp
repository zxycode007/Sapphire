...
namespace Sapphire
{
		...
		FileNameText(0), FileList(0), Dragging(false)
	{
		...
		if (FileSystem)
		{
			...
			if (!startDir.empty())
			{
				StartDirectory = StringUtil::StringWToString(startDir);
				FileSystem->changeWorkingDirectoryTo(StringUtil::StringWToString(startDir));
			}
		}
		else
			return;
		...
		const SINT32 buttonw = skin->getSize(EGDS_WINDOW_BUTTON_WIDTH);
		...
		FileBox = Environment->addListBox(rect<SINT32>(10, 55, RelativeRect.getWidth() - 90, 230), this, -1, true);
		FileBox->setSubElement(true);
		FileBox->setAlignment(EGUIA_UPPERLEFT, EGUIA_LOWERRIGHT, EGUIA_UPPERLEFT, EGUIA_LOWERRIGHT);
		FileBox->grab();

		FileNameText = Environment->addEditBox(0, rect<SINT32>(10, 30, RelativeRect.getWidth() - 90, 50), true, this);
	...
	const path& CGUIFileOpenDialog::getDirectoryName()
	{
	...
	bool CGUIFileOpenDialog::OnEvent(const SEvent& event)
	{
		if (isEnabled())
		{
			...
			case EET_GUI_EVENT:
				...
				{
					SINT32 selected = FileBox->getSelected();
					if (FileList && FileSystem)
					{
						...
						else
						{
							//FileDirectory = L"";
							FileDirectory = "";
							FileName = StringUtil::StringToStringW(FileList->getFullFileName(selected));
						}
						return true;
					}
				}
				...
				{
					const SINT32 selected = FileBox->getSelected();
					if (FileList && FileSystem)
					{
						if (FileList->isDirectory(selected))
						{
				...
				case EGET_EDITBOX_ENTER:
					...
					{
						path dir(StringUtil::StringWToString(FileNameText->getText()));
	...
	void CGUIFileOpenDialog::fillListBox()
	{
...
#if !defined(_SAPPHIRE_WINDOWS_CE_PLATFORM_)
		setlocale(LC_ALL, "");
#endif
		...
		if (FileList)
		{
			...
			{
#ifndef _SAPPHIRE_WCHAR_FILESYSTEM
		...
		if (FileNameText)
		{
			...
			FileDirectory = StringUtil::StringWToString(s);
