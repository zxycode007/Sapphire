#ifndef _SAPPHIRE_I_GUI_BUTTON_
#define _SAPPHIRE_I_GUI_BUTTON_
...
namespace Sapphire
{
	...
	*/
	...
	public:
		...
		//! constructor
		IGUIButton(IGUIEnvironment* environment, IGUIElement* parent, SINT32 id, rect<SINT32> rectangle)
		...
		virtual void setImage(ITexture* image = 0) = 0;

		//! Sets a background image for the button when it is in normal state.
		/** \param image: Texture containing the image to be displayed
		\param pos: Position in the texture, where the image is located */
		virtual void setImage(ITexture* image, const rect<SINT32>& pos) = 0;

		//! Sets a background image for the button when it is in pressed state.
		/** If no images is specified for the pressed state via
		setPressedImage(), this image is also drawn in pressed state.
		\param image: Image to be displayed */
		virtual void setPressedImage(ITexture* image = 0) = 0;

		//! Sets an image which should be displayed on the button when it is in pressed state.
		/** \param image: Texture containing the image to be displayed
		\param pos: Position in the texture, where the image is located */
		virtual void setPressedImage(ITexture* image, const rect<SINT32>& pos) = 0;
		...
		*/
		virtual void setSprite(EGUI_BUTTON_STATE state, SINT32 index,
			ColourValue color = ColourValue(255, 255, 255, 255), bool loop = false) = 0;
