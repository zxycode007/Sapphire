...
namespace Sapphire
{
	...
	CGUIButton::CGUIButton(IGUIEnvironment* environment, IGUIElement* parent,
		SINT32 id, rect<SINT32> rectangle, bool noclip)
	...
	void CGUIButton::setSprite(EGUI_BUTTON_STATE state, SINT32 index, ColourValue color, bool loop)
	{
		if (SpriteBank)
		{
			ButtonSprites[(UINT32)state].Index = index;
			ButtonSprites[(UINT32)state].Color = color;
			ButtonSprites[(UINT32)state].Loop = loop;
		}
		else
		{
			ButtonSprites[(UINT32)state].Index = -1;
		}
	}
	...
	void CGUIButton::setImage(ITexture* image, const rect<SINT32>& pos)
	{
		setImage(image);
		ImageRect = pos;
	}
