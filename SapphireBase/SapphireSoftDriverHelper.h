#ifndef _SAPPHIRE_SOFT_DRIVER_HELPER_
#define _SAPPHIRE_SOFT_DRIVER_HELPER_
...
#define VIDEO_SAMPLE_GRANULARITY	1
	...
	static inline SINT32 SINT32_log2_FLOAT32( FLOAT32 f)
	{
		UINT32 x = IR ( f );
		return ((x & 0x7F800000) >> 23) - 127;
	}

	static inline SINT32 SINT32_log2_SINT32(UINT32 x)
	{
		return SINT32_log2_FLOAT32( (FLOAT32) x);
	}

	static inline SINT32 SINT32_abs(SINT32 x)
	{
		SINT32 b = x >> 31;
		return (x ^ b ) - b;
	}
	...
	FORCEINLINE UINT32 if_mask_a_else_b ( const UINT32 mask, const UINT32 a, const UINT32 b )
	{
		return ( mask & ( a ^ b ) ) ^ b;
	}
	...
	alpha [0;256]
	*/
	FORCEINLINE UINT32 PixelBlend32(const UINT32 c2, const UINT32 c1, UINT32 alpha)
	{
	...
	*/
	inline UINT16 PixelMul16 ( const UINT16 c0, const UINT16 c1)
	{
	...
	*/
	FORCEINLINE UINT32 PixelMul32(const UINT32 c0, const UINT32 c1)
	{
	...
	Pixel = c0 * (c1/255).
	*/
	FORCEINLINE UINT32 PixelMul32_2(const UINT32 c0, const UINT32 c1)
	{
	...
	*/
	FORCEINLINE UINT32 PixelAdd32(const UINT32 c2, const UINT32 c1)
	{
		UINT32 sum = (c2 & 0x00FFFFFF) + (c1 & 0x00FFFFFF);
		UINT32 low_bits = (c2 ^ c1) & 0x00010101;
		SINT32 carries = (sum - low_bits) & 0x01010100;
		UINT32 modulo = sum - carries;
		UINT32 clamp = carries - (carries >> 8);
		return modulo | clamp;
	}
	...
	inline UINT16 PixelBlend16(const UINT16 destination, const UINT16 source)
	{
	...
	inline UINT32 PixelBlend16_simd(const UINT32 destination, const UINT32 source)
	{
	...
	*/
	...
	typedef SINT32 tFixPoint;
	typedef UINT32 tFixPointu;
...
#define FIX_POINT_HALF_COLOR ( (tFixPoint) ( ((FLOAT32) COLOR_MAX / 2.f * FIX_POINT_F32_MUL ) ) )


	/*
	convert signed integer to fixpoint
	*/
	inline tFixPoint SINT32_to_fixPoint (const SINT32 x)
	{
		return x << FIX_POINT_PRE;
	}

	inline tFixPointu UINT32_to_fixPoint (const UINT32 x)
	{
		return x << FIX_POINT_PRE;
	}

	inline UINT32 fixPointu_to_UINT32 (const tFixPointu x)
	{
		return x >> FIX_POINT_PRE;
	}
	...
	// 1/x * FIX_POINT
	FORCEINLINE FLOAT32 fix_inverse32 ( const FLOAT32 x )
	{
	...
	*/
	static inline int f_round2(FLOAT32 f)
	{
		f += (3 << 22);
	...
	/*
	convert FLOAT32 to Fix Point.
	multiply is needed anyway, so scale mulby
	*/
	FORCEINLINE tFixPoint tofix(const FLOAT32 x, const FLOAT32 mulby = FIX_POINT_F32_MUL)
	{
	...
	*/
	FORCEINLINE tFixPointu imulFixu(const tFixPointu x, const tFixPointu y)
	{
	...
	Fix Point , Fix Point Multiply
	*/
	FORCEINLINE tFixPoint imulFix(const tFixPoint x, const tFixPoint y)
	{
	...
	*/
	FORCEINLINE tFixPoint imulFix2(const tFixPoint x, const tFixPoint y)
	{
	...
	Multiply x * y * 1
	*/
	FORCEINLINE tFixPoint imulFix_tex1(const tFixPoint x, const tFixPoint y)
	{
	...
	Multiply x * y * 2
	*/
	FORCEINLINE tFixPoint imulFix_tex2(const tFixPoint x, const tFixPoint y)
	{
	...
	Multiply x * y * 4
	*/
	FORCEINLINE tFixPoint imulFix_tex4(const tFixPoint x, const tFixPoint y)
	{
#ifdef SOFTWARE_DRIVER_2_32BIT
	...
	*/
	FORCEINLINE tFixPoint clampfix_maxcolor(const tFixPoint a)
	{
	...
	*/
	FORCEINLINE tFixPoint clampfix_mincolor(const tFixPoint a)
	{
		return a - (a & (a >> 31));
	}

	FORCEINLINE tFixPoint saturateFix(const tFixPoint a)
	{
	...
	inline SINT32 roundFix(const tFixPoint x)
	{
	...
	// x in [0;1[
	inline SINT32 FLOAT32_to_23Bits(const FLOAT32 x)
	{
		FLOAT32 y = x + 1.f;
	...
	*/
	FORCEINLINE tVideoSample fix_to_color(const tFixPoint r, const tFixPoint g, const tFixPoint b)
	{
	...
	return VideoSample from fixpoint
	*/
	FORCEINLINE tVideoSample fix4_to_color(const tFixPoint a, const tFixPoint r, const tFixPoint g, const tFixPoint b)
	{
#ifdef __BIG_ENDIAN__
	...
	*/
	...
	struct fp24
	{
		...
		fp24 ( const FLOAT32 f )
		{
			FLOAT32 y = f + 1.f;
			v = ((UINT32&)y) & 0x7FFFFF;	// last 23 bits
		}
	...
	inline tVideoSample getTexel_plain ( const sInternalTexture * t, const tFixPointu tx, const tFixPointu ty )
	{
		...
		// texel
		return *((tVideoSample*)( (UINT8*) t->data + ofs ));
	}
		...
		)
	{
		...
		// texel
		tVideoSample t00;
		t00 = *((tVideoSample*)( (UINT8*) t->data + ofs ));
	...
	FORCEINLINE void getTexel_fix ( tFixPoint &a,
		const sInternalTexture * t, const tFixPointu tx, const tFixPointu ty
		)
	{
		UINT32 ofs;
		...
		)
	{
		...
		// texel
		const tVideoSample t00 = *((tVideoSample*)((UINT8*)t->data + ofs));
	...
	*/
	...
	// get sample linear
	FORCEINLINE void getSample_linear(tFixPointu &r, tFixPointu &g, tFixPointu &b,
		const sInternalTexture * t, const tFixPointu tx, const tFixPointu ty
		)
	{
		UINT32 ofs;
	...
	// get Sample bilinear
	FORCEINLINE void getSample_texture(tFixPoint &r, tFixPoint &g, tFixPoint &b,
		const sInternalTexture * t, const tFixPointu tx, const tFixPointu ty
		)
	{
	...
	// get sample linear
	FORCEINLINE void getSample_linear(tFixPointu &a, tFixPointu &r, tFixPointu &g, tFixPointu &b,
		const sInternalTexture * t, const tFixPointu tx, const tFixPointu ty
		)
	{
		UINT32 ofs;
	...
	// get Sample bilinear
	FORCEINLINE void getSample_texture(tFixPoint &a, tFixPoint &r, tFixPoint &g, tFixPoint &b,
		const sInternalTexture * t, const tFixPointu tx, const tFixPointu ty
		)
	{
	...
	{
		dest.x0 = SINT32_max(a.x0, b.x0);
		dest.y0 = SINT32_max(a.y0, b.y0);
		dest.x1 = SINT32_min(a.x1, b.x1);
		dest.y1 = SINT32_min(a.y1, b.y1);
	...
	inline SINT32 intervall_intersect_test(const sIntervall& a, const sIntervall& b)
	{
		return SINT32_min(a.end, b.end) - SINT32_max(a.start, b.start);
	}
