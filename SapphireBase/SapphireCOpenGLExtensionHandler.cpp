...
namespace Sapphire
{
...
#endif
#endif // SAPPHIRE_OPENGL_USE_EXTPOINTER
	{
		for (UINT32 i = 0; i<SAPPHIRE_OpenGL_Feature_Count; ++i)
	...
	{
		for (UINT32 i = 0; i<SAPPHIRE_OpenGL_Feature_Count; ++i)
			Printer::log(OpenGLFeatureStrings[i], FeatureAvailable[i] ? " true" : " false");
	}
...
#define SAPPHIRE_OGL_LOAD_EXTENSION(X) glXGetProcAddressARB(reinterpret_cast<const GLubyte*>(X))
	...
	bool COpenGLExtensionHandler::queryFeature(E_VIDEO_DRIVER_FEATURE feature) const
	{
		...
		case EVDF_MIP_MAP_AUTO_UPDATE:
			return FeatureAvailable[SAPPHIRE_SGIS_generate_mipmap] || FeatureAvailable[SAPPHIRE_EXT_framebuffer_object] || FeatureAvailable[SAPPHIRE_ARB_framebuffer_object];
		case EVDF_STENCIL_BUFFER:
			return StencilBuffer;
		case EVDF_VERTEX_SHADER_1_1:
		case EVDF_ARB_VERTEX_PROGRAM_1:
			return FeatureAvailable[SAPPHIRE_ARB_vertex_program] || FeatureAvailable[SAPPHIRE_NV_vertex_program1_1];
		case EVDF_PIXEL_SHADER_1_1:
		case EVDF_PIXEL_SHADER_1_2:
		case EVDF_ARB_FRAGMENT_PROGRAM_1:
			return FeatureAvailable[SAPPHIRE_ARB_fragment_program] || FeatureAvailable[SAPPHIRE_NV_fragment_program];
		case EVDF_PIXEL_SHADER_2_0:
		case EVDF_VERTEX_SHADER_2_0:
		case EVDF_ARB_GLSL:
			return (FeatureAvailable[SAPPHIRE_ARB_shading_language_100] || Version >= 200);
		...
		case EVDF_TEXTURE_NPOT:
			...
			// extra test for now!
			// return (FeatureAvailable[SAPPHIRE_ARB_texture_non_power_of_two]||Version>=200);
			return (FeatureAvailable[SAPPHIRE_ARB_texture_non_power_of_two]);
		case EVDF_FRAMEBUFFER_OBJECT:
			return FeatureAvailable[SAPPHIRE_EXT_framebuffer_object] || FeatureAvailable[SAPPHIRE_ARB_framebuffer_object];
		case EVDF_VERTEX_BUFFER_OBJECT:
			return FeatureAvailable[SAPPHIRE_ARB_vertex_buffer_object];
		case EVDF_COLOR_MASK:
			return true;
		case EVDF_ALPHA_TO_COVERAGE:
			return FeatureAvailable[SAPPHIRE_ARB_multisample];
		case EVDF_GEOMETRY_SHADER:
			return FeatureAvailable[SAPPHIRE_ARB_geometry_shader4] || FeatureAvailable[SAPPHIRE_EXT_geometry_shader4] || FeatureAvailable[SAPPHIRE_NV_geometry_program4] || FeatureAvailable[SAPPHIRE_NV_geometry_shader4];
		case EVDF_MULTIPLE_RENDER_TARGETS:
			return FeatureAvailable[SAPPHIRE_ARB_draw_buffers] || FeatureAvailable[SAPPHIRE_ATI_draw_buffers];
		case EVDF_MRT_BLEND:
		case EVDF_MRT_COLOR_MASK:
			return FeatureAvailable[SAPPHIRE_EXT_draw_buffers2];
		case EVDF_MRT_BLEND_FUNC:
			return FeatureAvailable[SAPPHIRE_ARB_draw_buffers_blend] || FeatureAvailable[SAPPHIRE_AMD_draw_buffers_blend];
		case EVDF_OCCLUSION_QUERY:
			return FeatureAvailable[SAPPHIRE_ARB_occlusion_query] && OcclusionQuerySupport;
		case EVDF_POLYGON_OFFSET:
			// both features supported with OpenGL 1.1
			return Version >= 110;
		case EVDF_BLEND_OPERATIONS:
			return (Version >= 120) || FeatureAvailable[SAPPHIRE_EXT_blend_minmax] ||
				FeatureAvailable[SAPPHIRE_EXT_blend_subtract] || FeatureAvailable[SAPPHIRE_EXT_blend_logic_op];
