...
namespace Sapphire
{
	...
	ISceneNode* CDefaultSceneNodeFactory::addSceneNode(ESCENE_NODE_TYPE type, ISceneNode* parent)
	{
		...
		case ESNT_TERRAIN:
			return Manager->addTerrainSceneNode("", parent, -1,
				...
				ColourValue::getColourValue(255, 255, 255, 255),
				4, ETPS_17, 0, true);
		case ESNT_SKY_BOX:
		...
		case ESNT_MESH:
			return Manager->addMeshSceneNode(0, parent, -1, Vector3(),
				Vector3(), Vector3(1, 1, 1), true);
		case ESNT_LIGHT:
		...
		case ESNT_ANIMATED_MESH:
			return Manager->addAnimatedMeshSceneNode(0, parent, -1, Vector3(),
				Vector3(), Vector3(1, 1, 1), true);
	...
	UINT32 CDefaultSceneNodeFactory::getCreatableSceneNodeTypeCount() const
	{
	...
	ESCENE_NODE_TYPE CDefaultSceneNodeFactory::getCreateableSceneNodeType(UINT32 idx) const
	{
	...
	const c8* CDefaultSceneNodeFactory::getCreateableSceneNodeTypeName(UINT32 idx) const
	{
		if (idx<SupportedSceneNodeTypes.size())
	...
	{
		for (UINT32 i = 0; i<SupportedSceneNodeTypes.size(); ++i)
