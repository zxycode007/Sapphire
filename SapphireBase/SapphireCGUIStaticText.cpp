...
namespace Sapphire
{
	...
	CGUIStaticText::CGUIStaticText(const wchar_t* text, bool border,
		...
		OverrideColor(ColourValue::getColourValue(101, 255, 255, 255)), BGColor(ColourValue::getColourValue(101, 210, 210, 210)),
		...
		if (Text.size())
		{
			...
			if (font)
			{
				...
				else
				{
					...
					rect<SINT32> r = frameRect;
					SINT32 height = font->getDimension(L"A").Height + font->getKerningHeight();
					SINT32 totalHeight = height * BrokenText.size();
					...
					for (UINT32 i = 0; i<BrokenText.size(); ++i)
					{
						if (HAlign == EGUIA_LOWERRIGHT)
						{
	...
	void CGUIStaticText::setOverrideColor(ColourValue color)
	{
		OverrideColor = color;
	...
	//! Sets another color for the text.
	void CGUIStaticText::setBackgroundColor(ColourValue color)
	{
		BGColor = color;
	...
	ColourValue CGUIStaticText::getBackgroundColor() const
	{
		return BGColor;
	}
	...
	ColourValue CGUIStaticText::getOverrideColor() const
	{
	...
	void CGUIStaticText::breakText()
	{
		...
		SINT32 length = 0;
		SINT32 elWidth = RelativeRect.getWidth();
		if (Border)
			...
			for (SINT32 i = 0; i<size; ++i)
			{
				...
				if (isWhitespace || i == (size - 1))
				{
					if (word.size())
					{
						...
						const SINT32 whitelgth = font->getDimension(whitespace.c_str()).Width;
						const SINT32 wordlgth = font->getDimension(word.c_str()).Width;
						...
						if (wordlgth > elWidth)
						{
							...
							// break the word at
							int where = word.find(wchar_t(0x00AD));
							if (where != -1)
							{
								StringW first = word.substr(0, where);
								StringW second = word.substr(where, word.size() - where);
								BrokenText.push_back(line + first + L"-");
								const SINT32 secondLength = font->getDimension(second.c_str()).Width;
		...
		else
		{
			...
			for (SINT32 i = size; i >= 0; --i)
			{
				...
				if (c == L' ' || c == 0 || i == 0)
				{
					...
					if (c != 0)
						whitespace = StringW(&c, 1) + whitespace;
	...
	SINT32 CGUIStaticText::getTextHeight() const
	{
		IGUIFont* font = getActiveFont();
		if (!font)
			return 0;
	...
	SINT32 CGUIStaticText::getTextWidth() const
	{
		IGUIFont * font = getActiveFont();
		if (!font)
			return 0;
		...
		if (WordWrap)
		{
			...
			for (UINT32 line = 0; line < BrokenText.size(); ++line)
			{
				SINT32 width = font->getDimension(BrokenText[line].c_str()).Width;
