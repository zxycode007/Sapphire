...
#include "SapphireCColorConverter.h"

namespace Sapphire
{

	//! constructor for usual textures
	COpenGLTexture::COpenGLTexture(IImage* origImage, const path& name, void* mipmapData, COpenGLDriver* driver)
	...
	COpenGLTexture::COpenGLTexture(const path& name, COpenGLDriver* driver)
		: ITexture(name), ColorFormat(ECF_A8R8G8B8), Driver(driver), Image(0), MipImage(0),
		TextureName(0), InternalFormat(GL_RGBA), PixelFormat(GL_BGRA_EXT),
		...
		GLenum& type)
	{
		...
		default:
		{
			Printer::log("Unsupported texture format", LML_ERROR);
			internalformat = GL_RGBA8;
		}
		}
	...
	void COpenGLTexture::getImageValues(IImage* image)
	{
		if (!image)
		{
			Printer::log("No image for OpenGL texture.", LML_ERROR);
			return;
		}
		...
		{
			Printer::log("Invalid size of image for OpenGL Texture.", LML_ERROR);
			return;
		}

		const FLOAT32 ratio = (FLOAT32)ImageSize.Width / (FLOAT32)ImageSize.Height;
		if ((ImageSize.Width>Driver->MaxTextureSize) && (ratio >= 1.0f))
		{
			ImageSize.Width = Driver->MaxTextureSize;
			ImageSize.Height = (UINT32)(Driver->MaxTextureSize / ratio);
		}
		else if (ImageSize.Height>Driver->MaxTextureSize)
		{
			ImageSize.Height = Driver->MaxTextureSize;
			ImageSize.Width = (UINT32)(Driver->MaxTextureSize*ratio);
		}
	...
	void COpenGLTexture::uploadTexture(bool newTexture, void* mipmapData, UINT32 level)
	{
		// check which image needs to be uploaded
		IImage* image = level ? MipImage : Image;
		if (!image)
		{
			Printer::log("No image for OpenGL texture to upload", LML_ERROR);
			return;
		}
		...
		if (Driver->testGLError())
			Printer::log("Could not bind Texture", LML_ERROR);
...
#else
			HasMipMaps = false;
			Printer::log("Did not create OpenGL texture mip maps.", LML_INFORMATION);
#endif
			{
		...
		if (Driver->testGLError())
			Printer::log("Could not glTexImage2D", LML_ERROR);
	}


	//! lock function
	void* COpenGLTexture::lock(E_TEXTURE_LOCK_MODE mode, UINT32 mipmapLevel)
	{
		...
		if (!image || IsRenderTarget)
		{
			...
			if (mode != ETLM_WRITE_ONLY)
			{
				...
				if (!mipmapLevel && Driver->queryOpenGLFeature(COpenGLExtensionHandler::SAPPHIRE_MESA_pack_invert))
				...
				{
#ifdef GL_MESA_pack_invert
					if (Driver->queryOpenGLFeature(COpenGLExtensionHandler::SAPPHIRE_MESA_pack_invert))
						glPixelStorei(GL_PACK_INVERT_MESA, GL_FALSE);
					else
#endif
					{
						// opengl images are horizontally flipped, so we have to fix that here.
						const SINT32 pitch = image->getPitch();
	...
	const dimension2d<UINT32>& COpenGLTexture::getOriginalSize() const
	{
		return ImageSize;
	}
	...
	const dimension2d<UINT32>& COpenGLTexture::getSize() const
	{
		return TextureSize;
	}
	...
	UINT32 COpenGLTexture::getPitch() const
	{
		if (Image)
	...
	void COpenGLTexture::regenerateMipMapLevels(void* mipmapData)
	{
		...
		UINT32 i = 0;
		UINT8* target = static_cast<UINT8*>(mipmapData);
		do
		{
			if (width>1)
				width >>= 1;
			...
			++i;
			if (!target)
				target = new UINT8[width*height*Image->getBytesPerPixel()];
			...
			if (mipmapData)
			{
				mipmapData = static_cast<UINT8*>(mipmapData)+width*height*Image->getBytesPerPixel();
				target = static_cast<UINT8*>(mipmapData);
			}
	...
	COpenGLFBOTexture::COpenGLFBOTexture(const dimension2d<UINT32>& size,
		const path& name, COpenGLDriver* driver,
		...
		StencilRenderBuffer(0), UseStencil(useStencil)
	{
		...
		if (useStencil)
		{
			...
			if (Driver->queryOpenGLFeature(COpenGLExtensionHandler::SAPPHIRE_EXT_packed_depth_stencil))
			{
	...
	bool checkFBOStatus(COpenGLDriver* Driver)
	{
		...
		case GL_FRAMEBUFFER_INCOMPLETE_READ_BUFFER_EXT:
			Printer::log("FBO has invalid read buffer", LML_ERROR);
			break;

		case GL_FRAMEBUFFER_INCOMPLETE_DRAW_BUFFER_EXT:
			Printer::log("FBO has invalid draw buffer", LML_ERROR);
			break;

		case GL_FRAMEBUFFER_INCOMPLETE_ATTACHMENT_EXT:
			Printer::log("FBO has one or several incomplete image attachments", LML_ERROR);
			break;

		case GL_FRAMEBUFFER_INCOMPLETE_FORMATS_EXT:
			Printer::log("FBO has one or several image attachments with different internal formats", LML_ERROR);
			break;

		case GL_FRAMEBUFFER_INCOMPLETE_DIMENSIONS_EXT:
			Printer::log("FBO has one or several image attachments with different dimensions", LML_ERROR);
			break;

			// not part of fbo_object anymore, but won't harm as it is just a return value
#ifdef GL_FRAMEBUFFER_INCOMPLETE_DUPLICATE_ATTACHMENT_EXT
		case GL_FRAMEBUFFER_INCOMPLETE_DUPLICATE_ATTACHMENT_EXT:
			Printer::log("FBO has a duplicate image attachment", LML_ERROR);
			break;
#endif

		case GL_FRAMEBUFFER_INCOMPLETE_MISSING_ATTACHMENT_EXT:
			Printer::log("FBO missing an image attachment", LML_ERROR);
			break;

#ifdef GL_EXT_framebuffer_multisample
		case GL_FRAMEBUFFER_INCOMPLETE_MULTISAMPLE_EXT:
			Printer::log("FBO wrong multisample setup", LML_ERROR);
			break;
#endif

		case GL_FRAMEBUFFER_UNSUPPORTED_EXT:
			Printer::log("FBO format unsupported", LML_ERROR);
			break;
...
#endif
		Printer::log("FBO error", LML_ERROR);
