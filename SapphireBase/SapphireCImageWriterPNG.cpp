#include "SapphireCImageWriterPNG.h"

#ifdef SAPPHIRE_COMPILE_WITH_PNG_WRITER_
#include "SapphireCImageLoaderPNG.h"
...
#ifdef SAPPHIRE_COMPILE_WITH_LIBPNG_
#ifndef SAPPHIRE_USE_NON_SYSTEM_LIB_PNG_
#include <png.h> // use system lib png
#else // SAPPHIRE_USE_NON_SYSTEM_LIB_PNG_
#include "libpng/png.h" // °üº¬png lib
#endif // SAPPHIRE_USE_NON_SYSTEM_LIB_PNG_
#endif // SAPPHIRE_COMPILE_WITH_LIBPNG_
...
namespace Sapphire
{
	...
	{
		Printer::log("PNG fatal error", msg, LML_ERROR);
	...
	{
		Printer::log("PNG warning", msg, LML_WARNING);
	}
	...
	void PNGAPI user_write_data_fcn(png_structp png_ptr, png_bytep data, png_size_t length)
	{
		...
		IWriteFile* file = (IWriteFile*)png_get_io_ptr(png_ptr);
		check = (png_size_t)file->write((const void*)data, (UINT32)length);
	...
	bool CImageWriterPNG::isAWriteableFileExtension(const path& filename) const
	{
#ifdef SAPPHIRE_COMPILE_WITH_LIBPNG_
	...
	bool CImageWriterPNG::writeImage(IWriteFile* file, IImage* image, UINT32 param) const
	{
#ifdef SAPPHIRE_COMPILE_WITH_LIBPNG_
		if (!file || !image)
			return false;
		...
		if (!png_ptr)
		{
			Printer::log("PNGWriter: Internal PNG create write struct failure\n", file->getFileName(), LML_ERROR);
			return false;
		}
		...
		if (!info_ptr)
		{
			Printer::log("PNGWriter: Internal PNG create info struct failure\n", file->getFileName(), LML_ERROR);
		...
		SINT32 lineWidth = image->getDimension().Width;
		switch (image->getColorFormat())
		{
		case ECF_R8G8B8:
		case ECF_R5G6B5:
		...
		UINT8* tmpImage = new UINT8[image->getDimension().Height*lineWidth];
		if (!tmpImage)
		{
			Printer::log("PNGWriter: Internal PNG create image failure\n", file->getFileName(), LML_ERROR);
			png_destroy_write_struct(&png_ptr, &info_ptr);
			return false;
		}
		...
		UINT8** RowPointers = new png_bytep[image->getDimension().Height];
		if (!RowPointers)
		{
			Printer::log("PNGWriter: Internal PNG create row pointers failure\n", file->getFileName(), LML_ERROR);
			png_destroy_write_struct(&png_ptr, &info_ptr);
			delete[] tmpImage;
			return false;
		}
		...
		for (UINT32 i = 0; i<image->getDimension().Height; ++i)
		{
			RowPointers[i] = data;
			data += lineWidth;
		}
