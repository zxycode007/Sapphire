#ifndef _SAPPHIRE_MAP_
#define _SAPPHIRE_MAP_
...
namespace Sapphire
{
	...
	public:
		...
		class AccessClass
		{
			...
			friend class order_map<KeyType, ValueType>;
		...
		public:
			...
			operator ValueType()
			{
				...
				// Not found
				assert(node == 0) // access violation
		...
		private:
			...
			AccessClass(order_map& tree, const KeyType& key) : Tree(tree), Key(key) {}
		...
		void swap(order_map<KeyType, ValueType>& other)
		{
	...
	private:
		...
		explicit order_map(const order_map& src);
		order_map& operator = (const order_map& src);
