#ifndef _SAPPHIRE_I_FILE_LIST_
#define _SAPPHIRE_I_FILE_LIST_
...
namespace Sapphire
{
	...
	public:
		...
		virtual const path& getFileName(UINT32 index) const = 0;

		//! Gets the full name of a file in the list including the path, based on an index.
		/** \param index is the zero based index of the file which name should
		be returned. The index must be less than the amount getFileCount() returns.
		\return File name of the file. Returns 0 if an error occured. */
		virtual const path& getFullFileName(UINT32 index) const = 0;
		...
		virtual bool isDirectory(UINT32 index) const = 0;
		...
		virtual SINT32 findFile(const path& filename, bool isFolder = false) const = 0;
		...
		virtual UINT32 addItem(const path& fullPath, UINT32 offset, UINT32 size, bool isDirectory, UINT32 id = 0) = 0;
