...
#include <winuser.h>
#if defined(_SAPPHIRE_COMPILE_WITH_JOYSTICK_EVENTS_)
#ifdef _SAPPHIRE_COMPILE_WITH_DIRECTINPUT_JOYSTICK_
...
namespace Sapphire
{
#ifdef _SAPPHIRE_COMPILE_WITH_DIRECT3D_8_
	IVideoDriver* createDirectX8Driver(const SSapphireCreationParameters& params,
		IFileSystem* io, HWND window);
#endif

#ifdef _SAPPHIRE_COMPILE_WITH_DIRECT3D_9_
	IVideoDriver* createDirectX9Driver(const SSapphireCreationParameters& params,
		IFileSystem* io, HWND window);
#endif
	...
	struct SJoystickWin32Control
	{
...
#if defined(_SAPPHIRE_COMPILE_WITH_JOYSTICK_EVENTS_) && defined(_SAPPHIRE_COMPILE_WITH_DIRECTINPUT_JOYSTICK_)
		...
		vector<JoystickInfo>::type ActiveJoysticks;
#endif

		SJoystickWin32Control(CSapphireDeviceWin32* dev) : Device(dev)
		{
#if defined(_SAPPHIRE_COMPILE_WITH_JOYSTICK_EVENTS_) && defined(_SAPPHIRE_COMPILE_WITH_DIRECTINPUT_JOYSTICK_)
			DirectInputDevice = 0;
			if (DI_OK != (DirectInput8Create(GetModuleHandle(NULL), DIRECTINPUT_VERSION, IID_IDirectInput8, (void**)&DirectInputDevice, NULL)))
			{
				Printer::log("Could not create DirectInput8 Object", LML_WARNING);
				return;
			}
#endif
		}
		...
		{
#if defined(_SAPPHIRE_COMPILE_WITH_JOYSTICK_EVENTS_) && defined(_SAPPHIRE_COMPILE_WITH_DIRECTINPUT_JOYSTICK_)
			for (UINT32 joystick = 0; joystick < ActiveJoysticks.size(); ++joystick)
			{
		...
		void directInputAddJoystick(LPCDIDEVICEINSTANCE lpddi)
		{
			...
			{
				Printer::log("Could not create DirectInput device", LML_WARNING);
				return;
			}
			...
			{
				Printer::log("Could not set DirectInput device cooperative level", LML_WARNING);
				return;
			}

			if (FAILED(activeJoystick.lpdijoy->SetDataFormat(&c_dfDIJoystick2)))
			{
				Printer::log("Could not set DirectInput device data format", LML_WARNING);
				return;
			}

			if (FAILED(activeJoystick.lpdijoy->Acquire()))
			{
				Printer::log("Could not set DirectInput cooperative level", LML_WARNING);
				return;
			}
			...
			{
				Printer::log("Could not read DirectInput device state", LML_WARNING);
				return;
			}
			...
			for (UINT8 i = 0; i<(activeJoystick.devcaps.dwAxes) - caxis; i++)
			{
				if (i + caxis < 8)
		...
		void pollJoysticks()
		{
			...
			for (joystick = 0; joystick < ActiveJoysticks.size(); ++joystick)
			{
				...
				if (!FAILED(ActiveJoysticks[joystick].lpdijoy->GetDeviceState(sizeof(info), &info)))
				{
					...
					event.JoystickEvent.POV = (UINT16)info.rgdwPOV[0];
					...
					while (dxAxis < 6 && irrAxis <caps.dwAxes)
					{
						...
						if (axisFound)
						{
							SINT32 val = axisValue - 32768;

							if (val <-32767) val = -32767;
							if (val > 32767) val = 32767;
							event.JoystickEvent.Axis[irrAxis] = (SINT16)(val);
							irrAxis++;
						}
				...
				if (JOYERR_NOERROR == joyGetPosEx(ActiveJoysticks[joystick].Index, &info))
				{
					...
					case 6:
						event.JoystickEvent.Axis[SEvent::SJoystickEvent::AXIS_V] =
							(SINT16)((65535 * (info.dwVpos - caps.wVmin)) / (caps.wVmax - caps.wVmin) - 32768);

					case 5:
						event.JoystickEvent.Axis[SEvent::SJoystickEvent::AXIS_U] =
							(SINT16)((65535 * (info.dwUpos - caps.wUmin)) / (caps.wUmax - caps.wUmin) - 32768);

					case 4:
						event.JoystickEvent.Axis[SEvent::SJoystickEvent::AXIS_R] =
							(SINT16)((65535 * (info.dwRpos - caps.wRmin)) / (caps.wRmax - caps.wRmin) - 32768);

					case 3:
						event.JoystickEvent.Axis[SEvent::SJoystickEvent::AXIS_Z] =
							(SINT16)((65535 * (info.dwZpos - caps.wZmin)) / (caps.wZmax - caps.wZmin) - 32768);

					case 2:
						event.JoystickEvent.Axis[SEvent::SJoystickEvent::AXIS_Y] =
							(SINT16)((65535 * (info.dwYpos - caps.wYmin)) / (caps.wYmax - caps.wYmin) - 32768);

					case 1:
						event.JoystickEvent.Axis[SEvent::SJoystickEvent::AXIS_X] =
							(SINT16)((65535 * (info.dwXpos - caps.wXmin)) / (caps.wXmax - caps.wXmin) - 32768);
					}
...
#endif
#endif // _SAPPHIRE_COMPILE_WITH_JOYSTICK_EVENTS_
		}

		bool activateJoysticks(vector<SJoystickInfo>::type & joystickInfo)
		{
#if defined _SAPPHIRE_COMPILE_WITH_JOYSTICK_EVENTS_
#ifdef _SAPPHIRE_COMPILE_WITH_DIRECTINPUT_JOYSTICK_
			...
			{
				Printer::log("Could not enum DirectInput8 controllers", LML_WARNING);
				return false;
			}
			...
			for (joystick = 0; joystick < joystickInfo.size(); ++joystick)
			{
				...
				Printer::log(logString, LML_INFORMATION);
			}
...
namespace
{
	...
	{
		HWND hWnd;
		Sapphire::CSapphireDeviceWin32* irrDev;
	};
	Sapphire::list<SEnvMapper>::type EnvMap;
...
{
	Sapphire::list<SEnvMapper>::iterator it = EnvMap.begin();
...
Sapphire::CSapphireDeviceWin32* getDeviceFromHWnd(HWND hWnd)
{
	Sapphire::list<SEnvMapper>::iterator it = EnvMap.begin();
	for (; it != EnvMap.end(); ++it)
		if ((*it).hWnd == hWnd)
...
#define WHEEL_DELTA 120
	...
	Sapphire::CSapphireDeviceWin32* dev = 0;
	Sapphire::SEvent event;

	static Sapphire::SINT32 ClickCount = 0;
	...
	struct messageMap
	{
		Sapphire::SINT32 group;
		UINT winMessage;
		Sapphire::SINT32 irrMessage;
	};

	static messageMap mouseMap[] =
	{
		{ 0, WM_LBUTTONDOWN, Sapphire::EMIE_LMOUSE_PRESSED_DOWN },
		{ 1, WM_LBUTTONUP, Sapphire::EMIE_LMOUSE_LEFT_UP },
		{ 0, WM_RBUTTONDOWN, Sapphire::EMIE_RMOUSE_PRESSED_DOWN },
		{ 1, WM_RBUTTONUP, Sapphire::EMIE_RMOUSE_LEFT_UP },
		{ 0, WM_MBUTTONDOWN, Sapphire::EMIE_MMOUSE_PRESSED_DOWN },
		{ 1, WM_MBUTTONUP, Sapphire::EMIE_MMOUSE_LEFT_UP },
		{ 2, WM_MOUSEMOVE, Sapphire::EMIE_MOUSE_MOVED },
		{ 3, WM_MOUSEWHEEL, Sapphire::EMIE_MOUSE_WHEEL },
		{ -1, 0, 0 }
	};
	...
	if (m->group >= 0)
	{
		...
		event.EventType = Sapphire::EET_MOUSE_INPUT_EVENT;
		event.MouseInput.Event = (Sapphire::EMOUSE_INPUT_EVENT) m->irrMessage;
		...
		if (wParam & MK_MBUTTON)
			event.MouseInput.ButtonStates |= Sapphire::EMBSM_MIDDLE;
#if(_WIN32_WINNT >= 0x0500)
		if (wParam & MK_XBUTTON1)
			event.MouseInput.ButtonStates |= Sapphire::EMBSM_EXTRA1;
		if (wParam & MK_XBUTTON2)
			event.MouseInput.ButtonStates |= Sapphire::EMBSM_EXTRA2;
#endif
		...
		if (m->group == 3)
		{
			...
			event.MouseInput.Wheel = ((Sapphire::FLOAT32)((short)HIWORD(wParam))) / (Sapphire::FLOAT32)WHEEL_DELTA;
		}
		...
		if (dev)
		{
			...
			if (event.MouseInput.Event >= Sapphire::EMIE_LMOUSE_PRESSED_DOWN && event.MouseInput.Event <= Sapphire::EMIE_MMOUSE_PRESSED_DOWN)
			{
				UINT32 clicks = dev->checkSuccessiveClicks(event.MouseInput.X, event.MouseInput.Y, event.MouseInput.Event);
				if (clicks == 2)
				{
					event.MouseInput.Event = (Sapphire::EMOUSE_INPUT_EVENT)(Sapphire::EMIE_LMOUSE_DOUBLE_CLICK + event.MouseInput.Event - Sapphire::EMIE_LMOUSE_PRESSED_DOWN);
					dev->postEventFromUser(event);
				}
				else if (clicks == 3)
				{
					event.MouseInput.Event = (Sapphire::EMOUSE_INPUT_EVENT)(Sapphire::EMIE_LMOUSE_TRIPLE_CLICK + event.MouseInput.Event - Sapphire::EMIE_LMOUSE_PRESSED_DOWN);
					dev->postEventFromUser(event);
				}
			}
		}
	...
	case WM_KEYUP:
	{
		...
		event.EventType = Sapphire::EET_KEY_INPUT_EVENT;
		event.KeyInput.Key = (Sapphire::EKEY_CODE)wParam;
		event.KeyInput.PressedDown = (message == WM_KEYDOWN || message == WM_SYSKEYDOWN);

		const UINT MY_MAPVK_VSC_TO_VK_EX = 3; // MAPVK_VSC_TO_VK_EX should be in SDK according to MSDN, but isn't in mine.
		if (event.KeyInput.Key == Sapphire::KEY_SHIFT)
		{
			// this will fail on systems before windows NT/2000/XP, not sure _what_ will return there instead.
			event.KeyInput.Key = (Sapphire::EKEY_CODE)MapVirtualKey(((lParam >> 16) & 255), MY_MAPVK_VSC_TO_VK_EX);
		}
		if (event.KeyInput.Key == Sapphire::KEY_CONTROL)
		{
			event.KeyInput.Key = (Sapphire::EKEY_CODE)MapVirtualKey(((lParam >> 16) & 255), MY_MAPVK_VSC_TO_VK_EX);
			// some keyboards will just return LEFT for both - left and right keys. So also check extend bit.
			if (lParam & 0x1000000)
				event.KeyInput.Key = Sapphire::KEY_RCONTROL;
		}
		if (event.KeyInput.Key == Sapphire::KEY_MENU)
		{
			event.KeyInput.Key = (Sapphire::EKEY_CODE)MapVirtualKey(((lParam >> 16) & 255), MY_MAPVK_VSC_TO_VK_EX);
			if (lParam & 0x1000000)
				event.KeyInput.Key = Sapphire::KEY_RMENU;
		}
	...
	case WM_USER:
		event.EventType = Sapphire::EET_USER_EVENT;
		event.UserEvent.UserData1 = (Sapphire::SINT32)wParam;
		event.UserEvent.UserData2 = (Sapphire::SINT32)lParam;
		dev = getDeviceFromHWnd(hWnd);
...
namespace Sapphire
{
	...
	//! constructor
	CSapphireDeviceWin32::CSapphireDeviceWin32(const SSapphireCreationParameters& params)
		: CSapphireDeviceStub(params), HWnd(0), ChangedToFullScreen(false), Resized(false),
		ExternalWindow(false), Win32CursorControl(0), JoyControl(0)
	{
#ifdef _DEBUG
		//setDebugName("CSapphireDeviceWin32");
#endif
		...
		Printer::log(winversion.c_str(), LML_NORMAL);
		...
		{
			const fschar_t* ClassName = __TEXT("CSapphireDeviceWin32");
			...
			SINT32 windowLeft = (GetSystemMetrics(SM_CXSCREEN) - realWidth) / 2;
			SINT32 windowTop = (GetSystemMetrics(SM_CYSCREEN) - realHeight) / 2;
	...
	//! destructor
	CSapphireDeviceWin32::~CSapphireDeviceWin32()
	{
		delete JoyControl;
	...
	//! create the driver
	void CSapphireDeviceWin32::createDriver()
	{
		...
		case EDT_DIRECT3D8:
			...
			if (!VideoDriver)
			{
				Printer::log("Could not create DIRECT3D8 Driver.", LML_ERROR);
			}
#else
			Printer::log("DIRECT3D8 Driver was not compiled into this dll. Try another one.", LML_ERROR);
#endif // _SAPPHIRE_COMPILE_WITH_DIRECT3D_8_
		...
		case EDT_DIRECT3D9:
			...
			if (!VideoDriver)
			{
				Printer::log("Could not create DIRECT3D9 Driver.", LML_ERROR);
			}
#else
			Printer::log("DIRECT3D9 Driver was not compiled into this dll. Try another one.", LML_ERROR);
#endif // _SAPPHIRE_COMPILE_WITH_DIRECT3D_9_
		...
		case EDT_OPENGL:
			...
			if (!VideoDriver)
			{
				Printer::log("Could not create OpenGL driver.", LML_ERROR);
			}
#else
			Printer::log("OpenGL driver was not compiled in.", LML_ERROR);
#endif
			break;

		case EDT_SOFTWARE:

#ifdef _SAPPHIRE_COMPILE_WITH_SOFTWARE_
			switchToFullScreen();

			VideoDriver = createSoftwareDriver(CreationParams.WindowSize, CreationParams.Fullscreen, FileSystem, this);
#else
			Printer::log("Software driver was not compiled in.", LML_ERROR);
#endif

			break;

		case EDT_BURNINGSVIDEO:
#ifdef _SAPPHIRE_COMPILE_WITH_BURNINGSVIDEO_
			switchToFullScreen();

			VideoDriver = createBurningVideoDriver(CreationParams, FileSystem, this);
#else
			Printer::log("Burning's Video driver was not compiled in.", LML_ERROR);
#endif
			break;
		...
		default:
			Printer::log("Unable to create video driver of unknown type.", LML_ERROR);
			break;
		}
	}
	...
	bool CSapphireDeviceWin32::run()
	{
		...
		//_SAPPHIRE_IMPLEMENT_MANAGED_MARSHALLING_BUGFIX;
		return !Close;
	}


	//! Pause the current process for the minimum time allowed only to allow other processes to execute
	void CSapphireDeviceWin32::yield()
	{
		Sleep(1);
	}

	//! Pause execution and let other processes to run for a specified amount of time.
	void CSapphireDeviceWin32::sleep(UINT32 timeMs, bool pauseTimer)
	{
	...
	void CSapphireDeviceWin32::resizeIfNecessary()
	{
		...
		else
		{
			...
			getVideoDriver()->OnResize(dimension2du((UINT32)r.right, (UINT32)r.bottom));
	...
	void CSapphireDeviceWin32::setWindowCaption(const wchar_t* text)
	{
	...
	bool CSapphireDeviceWin32::present(IImage* image, void* windowId, rect<SINT32>* src)
	{
		HWND hwnd = HWnd;
		if (windowId)
		...
		if (dc)
		{
			...
			bi.bV4Height = -((SINT32)image->getDimension().Height);
	...
	void CSapphireDeviceWin32::closeDevice()
	{
		MSG msg;
	...
	bool CSapphireDeviceWin32::isWindowActive() const
	{
		//_SAPPHIRE_IMPLEMENT_MANAGED_MARSHALLING_BUGFIX;
	...
	bool CSapphireDeviceWin32::isWindowFocused() const
	{
	...
	bool CSapphireDeviceWin32::isWindowMinimized() const
	{
		WINDOWPLACEMENT plc;
	...
	bool CSapphireDeviceWin32::switchToFullScreen(bool reset)
	{
		if (!CreationParams.Fullscreen)
			return true;
		...
		case DISP_CHANGE_RESTART:
			Printer::log("Switch to fullscreen: The computer must be restarted in order for the graphics mode to work.", LML_ERROR);
			break;
		case DISP_CHANGE_BADFLAGS:
			Printer::log("Switch to fullscreen: An invalid set of flags was passed in.", LML_ERROR);
			break;
		case DISP_CHANGE_BADPARAM:
			Printer::log("Switch to fullscreen: An invalid parameter was passed in. This can include an invalid flag or combination of flags.", LML_ERROR);
			break;
		case DISP_CHANGE_FAILED:
			Printer::log("Switch to fullscreen: The display driver failed the specified graphics mode.", LML_ERROR);
			break;
		case DISP_CHANGE_BADMODE:
			Printer::log("Switch to fullscreen: The graphics mode is not supported.", LML_ERROR);
			break;
		default:
			Printer::log("An unknown error occured while changing to fullscreen.", LML_ERROR);
			break;
		}
		//_SAPPHIRE_IMPLEMENT_MANAGED_MARSHALLING_BUGFIX;
		return ret;
	...
	CSapphireDeviceWin32::CCursorControl* CSapphireDeviceWin32::getWin32CursorControl()
	{
	...
	//! by the gfx adapter.
	IVideoModeList* CSapphireDeviceWin32::getVideoModeList()
	{
		if (!VideoModeList->getVideoModeCount())
		{
			...
			{
				VideoModeList->addMode(dimension2d<UINT32>(mode.dmPelsWidth, mode.dmPelsHeight),
					mode.dmBitsPerPel);
			...
			if (EnumDisplaySettings(NULL, ENUM_CURRENT_SETTINGS, &mode))
				VideoModeList->setDesktop(mode.dmBitsPerPel, dimension2d<UINT32>(mode.dmPelsWidth, mode.dmPelsHeight));
		}
...
#define PRODUCT_ULTIMATE_E	0x00000047
	...
	void CSapphireDeviceWin32::getWindowsVersion(String& out)
	{
		...
		case VER_PLATFORM_WIN32_NT:
			...
			if (osvi.dwMajorVersion <= 4)
			{
				sprintf(tmp, "version %ld.%ld %s (Build %ld)",
					osvi.dwMajorVersion,
					osvi.dwMinorVersion,
					String(osvi.szCSDVersion).c_str(),
					osvi.dwBuildNumber & 0xFFFF);
			}
			else
			{
				sprintf(tmp, "%s (Build %ld)", String(osvi.szCSDVersion).c_str(),
					osvi.dwBuildNumber & 0xFFFF);
			}
	...
	void CSapphireDeviceWin32::OnResized()
	{
		Resized = true;
	}

	//! Sets if the window should be resizable in windowed mode.
	void CSapphireDeviceWin32::setResizable(bool resize)
	{
	...
	void CSapphireDeviceWin32::minimizeWindow()
	{
	...
	void CSapphireDeviceWin32::maximizeWindow()
	{
		WINDOWPLACEMENT wndpl;
		wndpl.length = sizeof(WINDOWPLACEMENT);
		GetWindowPlacement(HWnd, &wndpl);
	...
	void CSapphireDeviceWin32::restoreWindow()
	{
		WINDOWPLACEMENT wndpl;
		wndpl.length = sizeof(WINDOWPLACEMENT);
		GetWindowPlacement(HWnd, &wndpl);
	...
	bool CSapphireDeviceWin32::activateJoysticks(vector<SJoystickInfo>::type & joystickInfo)
	{
		if (JoyControl)
	...
	bool CSapphireDeviceWin32::setGammaRamp(FLOAT32 red, FLOAT32 green, FLOAT32 blue, FLOAT32 brightness, FLOAT32 contrast)
	{
		bool r;
		UINT16 ramp[3][256];
	...
	bool CSapphireDeviceWin32::getGammaRamp(FLOAT32 &red, FLOAT32 &green, FLOAT32 &blue, FLOAT32 &brightness, FLOAT32 &contrast)
	{
		bool r;
		UINT16 ramp[3][256];
	...
	void CSapphireDeviceWin32::clearSystemMessages()
	{
		MSG msg;
	...
	void CSapphireDeviceWin32::ReportLastWinApiError()
	{
	...
	// Convert an Sapphire texture to a Windows cursor
	// Based on http://www.codeguru.com/cpp/w-p/win32/cursors/article.php/c4529/
	HCURSOR CSapphireDeviceWin32::TextureToCursor(HWND hwnd, ITexture * tex, const rect<SINT32>& sourceRect, const Position2d &hotspot)
	{
		//
		...
		UINT32 bytesPerPixel = IImage::getBitsPerPixelFromFormat(format) / 8;
		UINT32 bytesLeftGap = sourceRect.UpperLeftCorner.x * bytesPerPixel;
		UINT32 bytesRightGap = tex->getPitch() - sourceRect.LowerRightCorner.x * bytesPerPixel;
		const UINT8* data = (const UINT8*)tex->lock(ETLM_READ_ONLY, 0);
		data += ((SINT32)(sourceRect.UpperLeftCorner.y+0.5)*tex->getPitch());
		for (SINT32 y = 0; y < sourceRect.getHeight(); ++y)
		{
			data += bytesLeftGap;
			for (SINT32 x = 0; x < sourceRect.getWidth(); ++x)
			{
				ColourValue pixelCol;
	...
	CSapphireDeviceWin32::CCursorControl::CCursorControl(CSapphireDeviceWin32* device, const dimension2d<UINT32>& wsize, HWND hwnd, bool fullscreen)
	...
	CSapphireDeviceWin32::CCursorControl::~CCursorControl()
	{
		for (UINT32 i = 0; i < Cursors.size(); ++i)
		{
			for (UINT32 f = 0; f < Cursors[i].Frames.size(); ++f)
			{
	...
	void CSapphireDeviceWin32::CCursorControl::initCursors()
	{
	...
	void CSapphireDeviceWin32::CCursorControl::update()
	{
		if (!Cursors[ActiveIcon].Frames.empty() && Cursors[ActiveIcon].FrameTime)
		{
			...
			UINT32 now = Device->getTimer()->getRealTime();
			UINT32 frame = ((now - ActiveIconStartTime) / Cursors[ActiveIcon].FrameTime) % Cursors[ActiveIcon].Frames.size();
			SetCursor(Cursors[ActiveIcon].Frames[frame].IconHW);
		}
	}

	//! Sets the active cursor icon
	void CSapphireDeviceWin32::CCursorControl::setActiveIcon(ECURSOR_ICON iconId)
	{
		if (iconId >= (SINT32)Cursors.size())
			return;
	...
	ECURSOR_ICON CSapphireDeviceWin32::CCursorControl::addIcon(const SCursorSprite& icon)
	{
		if (icon.SpriteId >= 0)
		{
			CursorW32 cW32;
			...
			for (UINT32 i = 0; i < icon.SpriteBank->getSprites()[icon.SpriteId].Frames.size(); ++i)
			{
				UINT32 texId = icon.SpriteBank->getSprites()[icon.SpriteId].Frames[i].textureNumber;
				UINT32 rectId = icon.SpriteBank->getSprites()[icon.SpriteId].Frames[i].rectNumber;
				rect<SINT32> rectIcon = icon.SpriteBank->getPositions()[rectId];
	...
	void CSapphireDeviceWin32::CCursorControl::changeIcon(ECURSOR_ICON iconId, const SCursorSprite& icon)
	{
		if (iconId >= (SINT32)Cursors.size())
			return;

		for (UINT32 i = 0; i < Cursors[iconId].Frames.size(); ++i)
	...
	dimension2di CSapphireDeviceWin32::CCursorControl::getSupportedIconSize() const
	{
		dimension2di result;
