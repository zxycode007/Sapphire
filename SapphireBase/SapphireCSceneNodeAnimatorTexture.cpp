...
namespace Sapphire
{
	...
	//! constructor
	CSceneNodeAnimatorTexture::CSceneNodeAnimatorTexture(const vector<ITexture*>::type& textures,
	...
	void CSceneNodeAnimatorTexture::animateNode(ISceneNode* node, UINT32 timeMs)
	{
		if (!node)
			return;
		...
		if (Textures.size())
		{
			const UINT32 t = (timeMs - StartTime);
	...
	void CSceneNodeAnimatorTexture::serializeAttributes(IAttributes* out, SAttributeReadWriteOptions* options) const
	{
		...
		UINT32 count = Textures.size();
		if (options && (options->Flags & EARWF_FOR_EDITOR))
			count += 1;

		for (UINT32 i = 0; i<count; ++i)
		{
			String tname = "Texture";
			tname += (int)(i + 1);
	...
	void CSceneNodeAnimatorTexture::deserializeAttributes(IAttributes* in, SAttributeReadWriteOptions* options)
	{
		...
		for (UINT32 i = 1; true; ++i)
		{
			...
			{
				ITexture* tex = in->getAttributeAsTexture(tname.c_str());
				if (tex)
				{
					tex->grab();
