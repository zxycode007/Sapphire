...
namespace Sapphire
{
	...
	//! constructor
	CMeshSceneNode::CMeshSceneNode(IMesh* mesh, ISceneNode* parent, ISceneManager* mgr, SINT32 id,
	...
	const AxisAlignedBox& CMeshSceneNode::getBoundingBox() const
	{
	...
	SMaterial& CMeshSceneNode::getMaterial(UINT32 i)
	{
	...
	UINT32 CMeshSceneNode::getMaterialCount() const
	{
	...
	{
		//path oldMeshStr = SceneManager->getMeshCache()->getMeshName(Mesh)
			...
			in->existsAttribute("HardwareMappingBufferType"))
		{
			...
			String smapping = in->getAttributeAsString("HardwareMappingHint");
			if (StringUtil::equals(smapping,"static",false) ) //if (smapping.equals_ignore_case("static")
				mapping = EHM_STATIC;
			else if (StringUtil::equals(smapping, "dynamic", false))  //else if (smapping.equals_ignore_case("dynamic"))
				mapping = EHM_DYNAMIC;
			else if (StringUtil::equals(smapping, "stream", false))  //else if (smapping.equals_ignore_case("stream"))
				mapping = EHM_STREAM;

			String sbufferType = in->getAttributeAsString("HardwareMappingBufferType");
			if (StringUtil::equals(sbufferType, "vertex", false))    //	if (sbufferType.equals_ignore_case("vertex"))
				bufferType = EBT_VERTEX;
			else if (StringUtil::equals(sbufferType, "index", false))  //else if (sbufferType.equals_ignore_case("index"))
				bufferType = EBT_INDEX;
			else if (StringUtil::equals(sbufferType, "vertexindex", false))  //else if (sbufferType.equals_ignore_case("vertexindex"))
