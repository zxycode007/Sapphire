...
namespace Sapphire
{
	CReadFile::CReadFile(const path& fileName)
	...
	SINT32 CReadFile::read(void* buffer, UINT32 sizeToRead)
	{
		if (!isOpen())
			return 0;

		return (SINT32)fread(buffer, 1, sizeToRead, File);
	}
	...
	const path& CReadFile::getFileName() const
	{
		return Filename;
	}
