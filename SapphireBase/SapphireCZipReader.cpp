...
{
	Sapphire::Printer::log("Error in bzip2 handling", Sapphire::StringUtil::int32ToString(errorCode), Sapphire::LML_ERROR);
}
...
#ifdef _SAPPHIRE_COMPILE_WITH_ZLIB_
#ifndef _SAPPHIRE_USE_NON_SYSTEM_ZLIB_
#include <zlib.h> // use system lib
#else
#include "zlib/zlib.h"
#endif

#ifdef _SAPPHIRE_COMPILE_WITH_ZIP_ENCRYPTION_
#include "aesGladman/fileenc.h"
#endif
#ifdef _SAPPHIRE_COMPILE_WITH_BZIP2_
#ifndef _SAPPHIRE_USE_NON_SYSTEM_BZLIB_
#include <bzlib.h>
#else
#include "bzip2/bzlib.h"
#endif
#endif
#ifdef _SAPPHIRE_COMPILE_WITH_LZMA_
...
namespace Sapphire
{
...
#if   (SAPPHIRE_CONFIG_ENDIAN == SAPPHIRE_BIG_ENDIAN)
		header.Sig = Byteswap::byteswap(header.Sig);
#endif
...
#if   (SAPPHIRE_CONFIG_ENDIAN == SAPPHIRE_BIG_ENDIAN)
			header.sig = Byteswap::byteswap(header.sig);
			header.time = Byteswap::byteswap(header.time);
#endif
...
#if   (SAPPHIRE_CONFIG_ENDIAN == SAPPHIRE_BIG_ENDIAN)
				dataLen = Byteswap::byteswap(dataLen);
#endif
...
#if (SAPPHIRE_CONFIG_ENDIAN == SAPPHIRE_BIG_ENDIAN)
			entry.header.DataDescriptor.CRC32 = Byteswap::byteswap(entry.header.DataDescriptor.CRC32);
			entry.header.DataDescriptor.UncompressedSize = Byteswap::byteswap(entry.header.DataDescriptor.UncompressedSize);
#endif
	...
	bool CZipReader::scanZipHeader(bool ignoreGPBits)
	{
...
#if (SAPPHIRE_CONFIG_ENDIAN == SAPPHIRE_BIG_ENDIAN)
		entry.header.Sig = Byteswap::byteswap(entry.header.Sig);
		entry.header.VersionToExtract = Byteswap::byteswap(entry.header.VersionToExtract);
		entry.header.GeneralBitFlag = Byteswap::byteswap(entry.header.GeneralBitFlag);
		entry.header.CompressionMethod = Byteswap::byteswap(entry.header.CompressionMethod);
		entry.header.LastModFileTime = Byteswap::byteswap(entry.header.LastModFileTime);
		entry.header.LastModFileDate = Byteswap::byteswap(entry.header.LastModFileDate);
		entry.header.DataDescriptor.CRC32 = Byteswap::byteswap(entry.header.DataDescriptor.CRC32);
		entry.header.DataDescriptor.CompressedSize = Byteswap::byteswap(entry.header.DataDescriptor.CompressedSize);
		entry.header.DataDescriptor.UncompressedSize = Byteswap::byteswap(entry.header.DataDescriptor.UncompressedSize);
		entry.header.FilenameLength = Byteswap::byteswap(entry.header.FilenameLength);
		entry.header.ExtraFieldLength = Byteswap::byteswap(entry.header.ExtraFieldLength);
#endif
		...
		{
			SINT16 restSize = entry.header.ExtraFieldLength;
			SZipFileExtraHeader extraHeader;
			while (restSize)
			{
				File->read(&extraHeader, sizeof(extraHeader));
#if (SAPPHIRE_CONFIG_ENDIAN == SAPPHIRE_BIG_ENDIAN)
				extraHeader.ID = Byteswap::byteswap(extraHeader.ID);
				extraHeader.Size = Byteswap::byteswap(extraHeader.Size);
#endif
				restSize -= sizeof(extraHeader);
				if (extraHeader.ID == (SINT16)0x9901)
				{
					SZipFileAESExtraData data;
					File->read(&data, sizeof(data));
#if (SAPPHIRE_CONFIG_ENDIAN == SAPPHIRE_BIG_ENDIAN)
					data.Version = Byteswap::byteswap(data.Version);
					data.CompressionMode = Byteswap::byteswap(data.CompressionMode);
#endif
...
#if   (SAPPHIRE_CONFIG_ENDIAN == SAPPHIRE_BIG_ENDIAN)
			dirEnd.NumberDisk = Byteswap::byteswap(dirEnd.NumberDisk);
			dirEnd.NumberStart = Byteswap::byteswap(dirEnd.NumberStart);
			dirEnd.TotalDisk = Byteswap::byteswap(dirEnd.TotalDisk);
			dirEnd.TotalEntries = Byteswap::byteswap(dirEnd.TotalEntries);
			dirEnd.Size = Byteswap::byteswap(dirEnd.Size);
			dirEnd.Offset = Byteswap::byteswap(dirEnd.Offset);
			dirEnd.CommentLength = Byteswap::byteswap(dirEnd.CommentLength);
#endif
			//SZipFileEntry s;
			//FileInfo.resize(dirEnd, s);
...
#ifdef _DEBUG
		//Debuginfo::print("added file from archive", ZipFileName.c_str());
#endif

		addItem(ZipFileName, entry.Offset, entry.header.DataDescriptor.UncompressedSize, ZipFileName[ZipFileName.length()-1] == '/', FileInfo.size());
		FileInfo.push_back(entry);
	...
	bool CZipReader::scanCentralDirectoryHeader()
	{
...
#if   (SAPPHIRE_CONFIG_ENDIAN == SAPPHIRE_BIG_ENDIAN)
		entry.Sig = Byteswap::byteswap(entry.Sig);
		entry.VersionMadeBy = Byteswap::byteswap(entry.VersionMadeBy);
		entry.VersionToExtract = Byteswap::byteswap(entry.VersionToExtract);
		entry.GeneralBitFlag = Byteswap::byteswap(entry.GeneralBitFlag);
		entry.CompressionMethod = Byteswap::byteswap(entry.CompressionMethod);
		entry.LastModFileTime = Byteswap::byteswap(entry.LastModFileTime);
		entry.LastModFileDate = Byteswap::byteswap(entry.LastModFileDate);
		entry.CRC32 = Byteswap::byteswap(entry.CRC32);
		entry.CompressedSize = Byteswap::byteswap(entry.CompressedSize);
		entry.UncompressedSize = Byteswap::byteswap(entry.UncompressedSize);
		entry.FilenameLength = Byteswap::byteswap(entry.FilenameLength);
		entry.ExtraFieldLength = Byteswap::byteswap(entry.ExtraFieldLength);
		entry.FileCommentLength = Byteswap::byteswap(entry.FileCommentLength);
		entry.DiskNumberStart = Byteswap::byteswap(entry.DiskNumberStart);
		entry.InternalFileAttributes = Byteswap::byteswap(entry.InternalFileAttributes);
		entry.ExternalFileAttributes = Byteswap::byteswap(entry.ExternalFileAttributes);
		entry.RelativeOffsetOfLocalHeader = Byteswap::byteswap(entry.RelativeOffsetOfLocalHeader);
#endif
		...
		FileInfo[FileInfo.size()-1].header.DataDescriptor.CompressedSize = entry.CompressedSize;
		FileInfo[FileInfo.size() - 1].header.DataDescriptor.UncompressedSize = entry.UncompressedSize;
		FileInfo[FileInfo.size() - 1].header.DataDescriptor.CRC32 = entry.CRC32;
		Files[FileInfo.size() - 1].Size = entry.UncompressedSize;
		return true;
	}
	...
	IReadFile* CZipReader::createAndOpenFile(UINT32 index)
	{
		...
		wchar_t buf[64];
		SINT16 actualCompressionMethod = e.header.CompressionMethod;
		IReadFile* decrypted = 0;
		UINT8* decryptedBuf = 0;
		UINT32 decryptedSize = e.header.DataDescriptor.CompressedSize;
#ifdef _SAPPHIRE_COMPILE_WITH_ZIP_ENCRYPTION_
		if ((e.header.GeneralBitFlag & ZIP_FILE_ENCRYPTED) && (e.header.CompressionMethod == 99))
		{
			Printer::log("Reading encrypted file.");
			UINT8 salt[16] = { 0 };
			const UINT16 saltSize = (((e.header.Sig & 0x00ff0000) >> 16) + 1) * 4;
			File->seek(e.Offset);
			...
			if (rc != 10)
			{
				Printer::log("Error on encryption closing");
				delete[] decryptedBuf;
				return 0;
			}
			...
			{
				Printer::log("Error on encryption check");
				delete[] decryptedBuf;
				return 0;
			}
			...
			{
				Printer::log("Unknown encryption method");
				return 0;
			}
#endif
		}
		...
		case 8:
		{
			...
			const UINT32 uncompressedSize = e.header.DataDescriptor.UncompressedSize;
			c8* pBuf = new c8[uncompressedSize];
			if (!pBuf)
			{
				swprintf(buf, 64, L"Not enough memory for decompressing %s", Files[index].FullName.c_str());
				Printer::log(StringUtil::StringWToString(buf), LML_ERROR);
				if (decrypted)
					decrypted->drop();
				return 0;
			}

			UINT8 *pcData = decryptedBuf;
			if (!pcData)
			{
				pcData = new UINT8[decryptedSize];
				if (!pcData)
				{
					swprintf(buf, 64, L"Not enough memory for decompressing %s", Files[index].FullName.c_str());
					Printer::log(StringUtil::StringWToString(buf), LML_ERROR);
		...
		case 12:
		{
			...
			*/
			...
			if (err != BZ_OK)
			{
				Printer::log("bzip2 decompression failed. File cannot be read.", LML_ERROR);
				return 0;
			}
...
#else
			Printer::log("bzip2 decompression not supported. File cannot be read.", LML_ERROR);
			return 0;
#endif
		}
		case 14:
		{
			...
			UINT32 uncompressedSize = e.header.DataDescriptor.UncompressedSize;
			c8* pBuf = new c8[uncompressedSize];
			if (!pBuf)
			{
				swprintf(buf, 64, L"Not enough memory for decompressing %s", Files[index].FullName.c_str());
			...
			if (err != SZ_OK)
			{
				Printer::log("Error decompressing", Files[index].FullName, LML_ERROR);
				delete[] pBuf;
				return 0;
			}
			else
...
#else
			Printer::log("lzma decompression not supported. File cannot be read.", LML_ERROR);
			return 0;
#endif
		}
		case 99:
			// If we come here with an encrypted file, decryption support is missing
			Printer::log("Decryption support not enabled. File cannot be read.",LML_ERROR);
			return 0;
		default:
