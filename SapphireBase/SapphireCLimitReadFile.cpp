...
namespace Sapphire
{
	...
	SINT32 CLimitReadFile::read(void* buffer, UINT32 sizeToRead)
	{
#if 1
		if (0 == File)
			return 0;

		SINT32 r = AreaStart + Pos;
		SINT32 toRead = SINT32_min(AreaEnd, r + sizeToRead) - SINT32_max(AreaStart, r);
		if (toRead < 0)
			return 0;
		File->seek(r);
	...
	{
#if 1
		Pos = SINT32_clamp(finalPos + (relativeMovement ? Pos : 0), 0, AreaEnd - AreaStart);
		return true;
#else
		const long pos = File->getPos();
