#ifndef _SAPPHIRE_S_CREATION_PARAMETER_
#define _SAPPHIRE_S_CREATION_PARAMETER_
...
namespace Sapphire
{
	...
	struct SSapphireCreationParameters
	{
		...
		SSapphireCreationParameters() :
			...
			WindowSize(dimension2d<UINT32>(800, 600)),
			Bits(16),
			ZBufferBits(16),
			Fullscreen(false),
			Stencilbuffer(false),
			...
			SDK_version_do_not_use(SAPPHIRE_VERSION)
		{
		}

		SSapphireCreationParameters(const SSapphireCreationParameters& other) :
			SDK_version_do_not_use(SAPPHIRE_VERSION)
		{
			*this = other;
		}

		SSapphireCreationParameters& operator=(const SSapphireCreationParameters& other)
		{
		...
		*/
		...
		const UINT32 const SDK_version_do_not_use;
	};
}
