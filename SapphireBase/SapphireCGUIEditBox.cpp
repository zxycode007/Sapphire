...
namespace Sapphire
{
	...
	CGUIEditBox::CGUIEditBox(const wchar_t* text, bool border,
		IGUIEnvironment* environment, IGUIElement* parent, SINT32 id,
		...
		OverrideColor(ColourValue(101, 255, 255, 255)), OverrideFont(0), LastBreakFont(0),
	...
	void CGUIEditBox::setOverrideColor(ColourValue color)
	{
		OverrideColor = color;
	...
	ColourValue CGUIEditBox::getOverrideColor() const
	{
	...
	{
		rect<SINT32> oldAbsoluteRect(AbsoluteRect);
	...
	bool CGUIEditBox::processKey(const SEvent& event)
	{
		...
		SINT32 newMarkBegin = MarkBegin;
		SINT32 newMarkEnd = MarkEnd;
		...
		if (event.KeyInput.Control)
		{
			...
			case KEY_KEY_C:
				...
				{
					const SINT32 realmbgn = MarkBegin < MarkEnd ? MarkBegin : MarkEnd;
					const SINT32 realmend = MarkBegin < MarkEnd ? MarkEnd : MarkBegin;

					String s;
					s = StringUtil::StringWToString( Text.substr(realmbgn, realmend - realmbgn));
			...
			case KEY_KEY_X:
				...
				if (!PasswordBox && Operator && MarkBegin != MarkEnd)
				{
					...
					// copy
					String sc;
					sc = StringUtil::StringWToString(Text.substr(realmbgn, realmend - realmbgn));
			...
			case KEY_KEY_V:
				...
				if (Operator)
				{
					...
					if (p)
					{
						...
						{
							// insert text
							StringW s = Text.substr(0, CursorPos);
							s.append(widep);
						...
						else
						{
							...
							StringW s = Text.substr(0, realmbgn);
							s.append(widep);
							s.append(Text.substr(realmend, Text.size() - realmend));
			...
			switch (event.KeyInput.Key)
		{
			case KEY_END:
			{
				...
				if (WordWrap || MultiLine)
				{
					...
					p = BrokenTextPositions[p] + (SINT32)BrokenText[p].size();
			...
			case KEY_RIGHT:
				...
				if (Text.size() > (UINT32)CursorPos) CursorPos++;
				BlinkStartTime = Timer::getTime();
				break;
			case KEY_UP:
				if (MultiLine || (WordWrap && BrokenText.size() > 1))
				{
					SINT32 lineNo = getLineFromPos(CursorPos);
					SINT32 mb = (MarkBegin == MarkEnd) ? CursorPos : (MarkBegin > MarkEnd ? MarkBegin : MarkEnd);
					if (lineNo > 0)
					{
						SINT32 cp = CursorPos - BrokenTextPositions[lineNo];
						if ((SINT32)BrokenText[lineNo - 1].size() < cp)
							CursorPos = BrokenTextPositions[lineNo - 1] + Math::_max((UINT32)1, BrokenText[lineNo - 1].size()) - 1;
						else
			...
			case KEY_DOWN:
				if (MultiLine || (WordWrap && BrokenText.size() > 1))
				{
					SINT32 lineNo = getLineFromPos(CursorPos);
					SINT32 mb = (MarkBegin == MarkEnd) ? CursorPos : (MarkBegin < MarkEnd ? MarkBegin : MarkEnd);
					if (lineNo < (SINT32)BrokenText.size() - 1)
					{
						SINT32 cp = CursorPos - BrokenTextPositions[lineNo];
						if ((SINT32)BrokenText[lineNo + 1].size() < cp)
							CursorPos = BrokenTextPositions[lineNo + 1] + Math::_max((UINT32)1, BrokenText[lineNo + 1].size()) - 1;
			...
			case KEY_DELETE:
				...
				if (Text.size() != 0)
				{
					...
					if (CursorPos > (SINT32)Text.size())
						CursorPos = (SINT32)Text.size();
	...
	void CGUIEditBox::draw()
	{
		...
		rect<SINT32> localClipRect = FrameRect;
		...
		if (font)
		{
			...
			const SINT32 realmbgn = MarkBegin < MarkEnd ? MarkBegin : MarkEnd;
			const SINT32 realmend = MarkBegin < MarkEnd ? MarkEnd : MarkBegin;
			const SINT32 hlineStart = ml ? getLineFromPos(realmbgn) : 0;
			const SINT32 hlineCount = ml ? getLineFromPos(realmend) - hlineStart + 1 : 1;
			const SINT32 lineCount = ml ? BrokenText.size() : 1;

			// Save the override color information.
			// Then, alter it if the edit box is disabled.
			const bool prevOver = OverrideColorEnabled;
			const ColourValue prevColor = OverrideColor;
			...
			if (Text.size())
			{
				...
				for (SINT32 i = 0; i < lineCount; ++i)
				{
					...
					if (PasswordBox)
					{
						...
						{
							BrokenText.clear();
							BrokenText.push_back(StringW());
						}
						if (BrokenText[0].size() != Text.size())
						{
							BrokenText[0] = Text;
							for (UINT32 q = 0; q < Text.size(); ++q)
							{
					...
					if (focus && MarkBegin != MarkEnd && i >= hlineStart && i < hlineStart + hlineCount)
					{
						...
						SINT32 mbegin = 0, mend = 0;
						SINT32 lineStartPos = 0, lineEndPos = txtLine->size();
			...
			if (IsEnabled)
			{
				...
				if (focus && (Timer::getTime() - BlinkStartTime) % 700 < 350)
				{
	...
	dimension2du CGUIEditBox::getTextDimension()
	{
		...
		for (UINT32 i = 1; i < BrokenText.size(); ++i)
		{
			setTextRect(i);
	...
	//! infinity.
	void CGUIEditBox::setMax(UINT32 max)
	{
		Max = max;
	...
	UINT32 CGUIEditBox::getMax() const
	{
		return Max;
	}
		...
		case EMIE_LMOUSE_PRESSED_DOWN:
			...
			else
			{
				if (!AbsoluteClippingRect.isPointInside(
					Position2d(event.MouseInput.X, event.MouseInput.Y)))
				{
					return false;
				}
				else
	...
	SINT32 CGUIEditBox::getCursorPos(SINT32 x, SINT32 y)
	{
		IGUIFont* font = getActiveFont();

		const UINT32 lineCount = (WordWrap || MultiLine) ? BrokenText.size() : 1;
		...
		SINT32 idx = font->getCharacterFromPos(txtLine->c_str(), x - CurrentTextRect.UpperLeftCorner.x);
	...
	void CGUIEditBox::breakText()
	{
		...
		StringW word;
		StringW whitespace;
		SINT32 lastLineStart = 0;
		SINT32 size = Text.size();
		SINT32 length = 0;
		SINT32 elWidth = RelativeRect.getWidth() - 6;
		wchar_t c;
		...
		for (SINT32 i = 0; i<size; ++i)
		{
			...
			if (c == L' ' || c == 0 || i == (size - 1))
			{
				...
				SINT32 whitelgth = font->getDimension(whitespace.c_str()).Width;
				SINT32 worldlgth = font->getDimension(word.c_str()).Width;
				...
				if (WordWrap && length + worldlgth + whitelgth > elWidth && line.size() > 0)
				{
					...
					lastLineStart = i - (SINT32)word.size();
					line = word;
				}
				else
				{
	...
	void CGUIEditBox::setTextRect(SINT32 line)
	{
		if (line < 0)
			return;
	...
	SINT32 CGUIEditBox::getLineFromPos(SINT32 pos)
	{
		if (!WordWrap && !MultiLine)
			return 0;

		SINT32 i = 0;
		while (i < (SINT32)BrokenTextPositions.size())
		{
			if (BrokenTextPositions[i] > pos)
				return i - 1;
			++i;
		}
		return (SINT32)BrokenTextPositions.size() - 1;
	}
	...
	void CGUIEditBox::calculateScrollPos()
	{
		...
		SINT32 cursLine = getLineFromPos(CursorPos);
		if (cursLine < 0)
			return;
		...
		// NOTE: Calculations different to vertical scrolling because setTextRect interprets VAlign relative to line but HAlign not relative to row
		{
			...
			// get cursor area
			UINT32 cursorWidth = font->getDimension(L"_").Width;
			StringW *txtLine = hasBrokenText ? &BrokenText[cursLine] : &Text;
			SINT32 cPos = hasBrokenText ? CursorPos - BrokenTextPositions[cursLine] : CursorPos;	// column
			SINT32 cStart = font->getDimension(txtLine->substr(0, cPos).c_str()).Width;		// pixels from text-start
			SINT32 cEnd = cStart + cursorWidth;
			SINT32 txtWidth = font->getDimension(txtLine->c_str()).Width;
		...
		if (hasBrokenText)
		{
			UINT32 lineHeight = font->getDimension(L"A").Height + font->getKerningHeight();
			// only up to 1 line fits?
			if (lineHeight >= (UINT32)FrameRect.getHeight())
			{
				VScrollPos = 0;
				setTextRect(cursLine);
				SINT32 unscrolledPos = CurrentTextRect.UpperLeftCorner.y;
				SINT32 pivot = FrameRect.UpperLeftCorner.y;
				switch (VAlign)
				{
				case EGUIA_CENTER:
			...
			else
			{
				...
				{
					UINT32 lastLine = BrokenTextPositions.empty() ? 0 : BrokenTextPositions.size() - 1;
	...
	//! set text markers
	void CGUIEditBox::setTextMarkers(SINT32 begin, SINT32 end)
	{
	...
	void CGUIEditBox::deserializeAttributes(IAttributes* in, SAttributeReadWriteOptions* options = 0)
	{
		...
		StringW ch = in->getAttributeAsStringW("PasswordChar");
