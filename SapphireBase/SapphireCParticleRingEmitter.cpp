...
namespace Sapphire
{
	...
	SINT32 CParticleRingEmitter::emitt(UINT32 now, UINT32 timeSinceLastCall, SParticle*& outArray)
	{
		Time += timeSinceLastCall;

		UINT32 pps = (MaxParticlesPerSecond - MinParticlesPerSecond);
		FLOAT32 perSecond = pps ? ((FLOAT32)MinParticlesPerSecond + Randomizer::frand() * pps) : MinParticlesPerSecond;
		FLOAT32 everyWhatMillisecond = 1000.0f / perSecond;
		...
		if (Time > everyWhatMillisecond)
		{
			...
			{
				FLOAT32 distance = Randomizer::frand() * RingThickness * 0.5f;
				...
				if (MaxAngleDegrees)
				{
					...
					tgt.rotateXYBy(Randomizer::frand() * MaxAngleDegrees, Center);
					tgt.rotateYZBy(Randomizer::frand() * MaxAngleDegrees, Center);
					tgt.rotateXZBy(Randomizer::frand() * MaxAngleDegrees, Center);
					p.vector = tgt;
				}
