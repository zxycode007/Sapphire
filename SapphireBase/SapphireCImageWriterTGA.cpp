#include "SapphireCImageWriterTGA.h"
...
namespace Sapphire
{
	...
	bool CImageWriterTGA::isAWriteableFileExtension(const path& filename) const
	{
		return  hasFileExtension(filename, "tga");
	}

	bool CImageWriterTGA::writeImage( IWriteFile *file, IImage *image, UINT32 param) const
	{
		...
		UINT32 row_stride = (pixel_size * imageHeader.ImageWidth);

		// length of one output row in bytes
		SINT32 row_size = ((imageHeader.PixelDepth / 8) * imageHeader.ImageWidth);
		...
		if (file->write(&imageFooter, sizeof(imageFooter)) < (SINT32)sizeof(imageFooter))
			return false;
