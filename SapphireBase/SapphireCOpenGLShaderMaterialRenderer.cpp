...
namespace Sapphire
{
	...
	COpenGLShaderMaterialRenderer::~COpenGLShaderMaterialRenderer()
	{
		...
		for (UINT32 i = 0; i<PixelShader.size(); ++i)
			if (PixelShader[i])
	...
	void COpenGLShaderMaterialRenderer::init(SINT32& outMaterialTypeNr,
...
#endif
		errString += StringUtil::int32ToString(SINT32(errPos));
		errString += ":\n";
#if defined(GL_ARB_vertex_program) || defined(GL_ARB_fragment_program)
...
#endif
#else
		String errString("Shaders not supported.");
#endif
		Printer::log(errString.c_str(), LML_ERROR);
		return true;
	}
	...
	bool COpenGLShaderMaterialRenderer::createPixelShader(const c8* pxsh)
	{
		...
		const String inshdr(pxsh);
		String shdr;
		const SINT32 pos = inshdr.find("#_IRR_FOG_MODE_");
		const UINT32 numShaders = (-1 != pos) ? 4 : 1;
