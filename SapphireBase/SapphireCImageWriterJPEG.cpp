#include "SapphireCImageWriterJPEG.h"
...
extern "C"
{
#ifndef SAPPHIRE_USE_NON_SYSTEM_JPEG_LIB_
#include <jpeglib.h>
#include <jerror.h>
#else
...
#define OUTPUT_BUF_SIZE 4096
	...
	{
		mem_dest_ptr dest = (mem_dest_ptr)cinfo->dest;
		const SINT32 datacount = (SINT32)(OUTPUT_BUF_SIZE - dest->pub.free_in_buffer);
		// for now just exit upon file error
	...
	// set up buffer data
	static void jpeg_file_dest(j_compress_ptr cinfo, IWriteFile* file)
	{
		if (cinfo->dest == NULL)
	...
	*/
	static bool writeJPEGFile(IWriteFile* file, IImage* image, UINT32 quality)
	{
		void(*format)(const void*, SINT32, void*) = 0;
		...
		const dimension2du dim = image->getDimension();
		...
		UINT8 * dest = new UINT8[dim.Width * 3];

		if (dest)
		{
			const UINT32 pitch = image->getPitch();
			JSAMPROW row_pointer[1];      /* pointer to JSAMPLE row[s] */
			row_pointer[0] = dest;

			UINT8* src = (UINT8*)image->lock();
	...
	bool CImageWriterJPG::isAWriteableFileExtension(const path& filename) const
	{
		return hasFileExtension(filename, "jpg", "jpeg");
	}


	bool CImageWriterJPG::writeImage(IWriteFile *file, IImage *image, UINT32 quality) const
	{
