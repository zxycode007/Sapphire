...
Sapphire::CSapphirDeviceConsole *DeviceToClose;
...
BOOL WINAPI ConsoleHandler(DWORD CEvent)
{
	...
	{
	case CTRL_C_EVENT:
		Sapphire::Printer::log("Closing console device", "CTRL+C");
		break;
	case CTRL_BREAK_EVENT:
		Sapphire::Printer::log("Closing console device", "CTRL+Break");
		break;
	case CTRL_CLOSE_EVENT:
		Sapphire::Printer::log("Closing console device", "User closed console");
		break;
	case CTRL_LOGOFF_EVENT:
		Sapphire::Printer::log("Closing console device", "User is logging off");
		break;
	case CTRL_SHUTDOWN_EVENT:
		Sapphire::Printer::log("Closing console device", "Computer shutting down");
		break;
	}
	DeviceToClose->closeDevice();
	return TRUE;
}
#elif defined(_SAPPHIRE_POSIX_API_)
// sigterm handler
#include <signal.h>

void sighandler(int sig)
{
	Sapphire::String code = "Signal ";
	code += sig;
	code += " received";
	Sapphire::Printer::log("Closing console device", code.c_str());
...
namespace Sapphire
{
	...
	const UINT16 ASCIIArtCharsCount = 32;

	//const c8 ASCIIArtChars[] = " \xb0\xb1\xf9\xb2\xdb";
	//const UINT16 ASCIIArtCharsCount = 5;

	//! constructor
	CSapphirDeviceConsole::CSapphirDeviceConsole(const SSapphireCreationParameters& params)
		: CSapphireDeviceStub(params), IsWindowFocused(true), ConsoleFont(0), OutFile(stdout)
	{
		DeviceToClose = this;
...
#ifdef _SAPPHIRE_VT100_CONSOLE_
		// reset terminal
		...
		case EDT_DIRECT3D9:
		case EDT_OPENGL:
			Printer::log("The console device cannot use hardware drivers yet.", LML_ERROR);
			break;
		case EDT_NULL:
		...
		for (UINT32 y = 0; y<CreationParams.WindowSize.Height; ++y)
		{
			String str;
			str.reserve(CreationParams.WindowSize.Width);
			for (UINT32 x = 0; x<CreationParams.WindowSize.Width; ++x)
				str += " ";
		...
		{
			createGUIAndScene();
#ifdef _SAPPHIRE_USE_CONSOLE_FONT_
			if (GUIEnvironment)
			{
				...
				if (skin)
				{
					for (UINT32 i = 0; i < gui::EGDF_COUNT; ++i)
	...
	//! destructor
	CSapphirDeviceConsole::~CSapphirDeviceConsole()
	{
	...
	void CSapphirDeviceConsole::yield()
	{
	...
	void CSapphirDeviceConsole::sleep(UINT32 timeMs, bool pauseTimer)
	{
	...
	bool CSapphirDeviceConsole::isWindowActive() const
	{
	...
	bool CSapphirDeviceConsole::isWindowFocused() const
	{
		return IsWindowFocused;
	}

	//! returns if window is minimized
	bool CSapphirDeviceConsole::isWindowMinimized() const
	{
		return false;
	}

	//! presents a surface in the client area
	bool CSapphirDeviceConsole::present(IImage* surface, void* windowId, rect<SINT32>* src)
	{

		if (surface)
		{
			for (UINT32 y = 0; y < surface->getDimension().Height; ++y)
			{
				for (UINT32 x = 0; x< surface->getDimension().Width; ++x)
				{
					// get average pixel
					UINT32 avg = surface->getPixel(x, y).getAverage() * (ASCIIArtCharsCount - 1);
					avg /= 255;
		...
		for (UINT32 i = 0; i< Text.size(); ++i)
		{
			...
			if (y < (s32)OutputBuffer.size() && y > 0)
				for (UINT32 c = 0; c < Text[i].Text.size() && c + Text[i].Pos.x < OutputBuffer[y].size(); ++c)
					//if (Text[i].Text[c] != ' ')
		...
		// draw output
		for (UINT32 y = 0; y<OutputBuffer.size(); ++y)
		{
	...
	void CSapphirDeviceConsole::closeDevice()
	{
	...
	void CSapphirDeviceConsole::setResizable(bool resize)
	{
		// do nothing
	}
	...
	//! Minimize the window.
	void CSapphirDeviceConsole::minimizeWindow()
	{
		// do nothing
	}
	...
	//! Maximize window
	void CSapphirDeviceConsole::maximizeWindow()
	{
		// do nothing
	}
	...
	void CSapphirDeviceConsole::restoreWindow()
	{
		// do nothing
	}


	void CSapphirDeviceConsole::setTextCursorPos(SINT16 x, SINT16 y)
	{
#ifdef _SAPPHIRE_WINDOWS_NT_CONSOLE_
		// move WinNT cursor
		COORD Position;
...
#elif defined(_SAPPHIRE_VT100_CONSOLE_)
		// send escape code
	...
	void CSapphirDeviceConsole::addPostPresentText(SINT16 X, SINT16 Y, const wchar_t *text)
	{
		SPostPresentText p;
		p.Text = StringUtil::StringWToString(text);
		p.Pos.x = X;
		p.Pos.y = Y;
		Text.push_back(p);
	}
